...
import co.id.artslv.repository.StasiunRepository;
//import co.id.artslv.repository.SubclassRepository;
...
//            Map<String, Subclass> subclassMap = subclasslist.stream().collect(Collectors.toMap(Subclass::getSubclasscode, p -> p));
//            List<Stasiun> stasiunlist = stasiunRepository.findAll();
//            Map<String, Stasiun> stasiunMap = stasiunlist.stream().collect(Collectors.toMap(Stasiun::getCode, p -> p));

            //Insert Transaction
            Transaction newtrans = new Transaction();
            newtrans.setBookcode(transactionRepository.getBookCode());
            newtrans.setPaycode(bookingdata.getPaycode());
            newtrans.setTransdate(bookingdata.getTransdatetime());
            newtrans.setDepartdate(bookingdata.getDepartdate());
            ...
            if (savetrans == null) {
                ...
                for (Pax pax : paxlist) {
                    ...
                    newtransdet.setTransactionpaycode(bookingdata.getPaycode());
                    ...
                    Inventory inventory = inventoryRepository.findOne(pax.getInventoryid());
                    if(inventory == null){
